...
PATCH = 256
LR = 1e-3
LR_D = 1e-4          # discriminator lower LR (standard for restoration GANs)
WD = 1e-3
WARMUP = 500
GRAD_CLIP = 1.0
LOG_EVERY = 500
...
def main():
    ...
    disc = PatchDiscriminator(in_ch=6).to(DEV).train()
    ...
    it = iter(loader)
    print(f"[p2] gen params={sum(p.numel() for p in gen.parameters()):,}  disc params="
    ...
    for step in range(ITERS):
        ...
        if step % LOG_EVERY == 0 or step == ITERS - 1:
            ...
            print(f"[p2] it={step:5d} l1={l1.item():.4f} lpips={lp.item():.4f} "
                  f"blk={blk.item():.4f} ldl={ldl.item():.4f} adv={adv.item():.4f} "
                  f"d={d_loss.item():.4f}  {1000*el/max(1,step+1):.0f}ms/it  eta {eta/60:.1f}min",
                  flush=True)

    with ema.swap(gen):
        torch.save({"model": gen.state_dict(), "ema": ema.state_dict()}, OUT / "student_p2.pt")
        with torch.no_grad():
            if CLASSIC5.exists():
                res = eval_classic5(gen)
                for qf, (p_, pb) in res.items():
                    fb_p, fb_pb = FBCNN[qf]
                    print(f"[p2] Classic5 QF{qf}: PSNR={p_:.2f} ({p_-fb_p:+.2f} vs FBCNN)  "
            ...
            contact_sheet(sheet, cols=2, thumb=320, path=str(OUT / "contact_sheet_p2.png"))
            print("[p2] contact sheet -> contact_sheet_p2.png", flush=True)
    ...
    print(f"[p2] DONE in {(time.time()-t0)/60:.1f}min  -> {OUT}", flush=True)
